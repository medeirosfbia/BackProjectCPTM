...
using ApiOracle.Models; // Inspecao e InspecaoFoto
...
namespace ApiOracle.Repositories
{
    public class InspecaoRepository
    {
        ...
        public async Task CriarTabelaAsync()
        {
            ...
            BEGIN
                EXECUTE IMMEDIATE '
                    CREATE TABLE INSPECOES_FOTOS (
                        ID NUMBER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
                        INSPECAO_ID NUMBER NOT NULL,
                        IMAGEM BLOB NOT NULL,
                        CONTENT_TYPE VARCHAR2(200) NOT NULL,
                        FILE_NAME VARCHAR2(400),
                        CREATED_AT DATE NOT NULL,
                        CONSTRAINT FK_INSPECOES_FOTOS_INSPECAO
            ...
            await conn.ExecuteAsync(fotosSql);
        }
        ...
        public async Task<int> InserirFotoAsync(int inspecaoId, byte[] imagem, string contentType, string? fileName, DateTime createdAt)
        {
            using var conn = _factory.CreateConnection();

            var sql = @"
                INSERT INTO INSPECOES_FOTOS (INSPECAO_ID, IMAGEM, CONTENT_TYPE, FILE_NAME, CREATED_AT)
                VALUES (:InspecaoId, :Imagem, :ContentType, :FileName, :CreatedAt)
                RETURNING ID INTO :Id";

            var parameters = new DynamicParameters();
            parameters.Add("InspecaoId", inspecaoId);
            parameters.Add("Imagem", imagem);
        ...
        public async Task<IEnumerable<InspecaoFoto>> ListarFotosAsync(int inspecaoId)
        {
            using var conn = _factory.CreateConnection();
            ...
            var sql = @"
                SELECT
                    ID,
                    INSPECAO_ID AS InspecaoId,
                    IMAGEM AS Imagem,
                    CONTENT_TYPE AS ContentType,
                    FILE_NAME AS FileName,
                    CREATED_AT AS CreatedAt
                FROM INSPECOES_FOTOS
                WHERE INSPECAO_ID = :InspecaoId
                ORDER BY CREATED_AT ASC, ID ASC";

            return await conn.QueryAsync<InspecaoFoto>(sql, new { InspecaoId = inspecaoId });
        }

        public async Task<InspecaoFoto?> ObterFotoAsync(int fotoId)
        {
            using var conn = _factory.CreateConnection();
            ...
            return await conn.QueryFirstOrDefaultAsync<InspecaoFoto>(sql, new { Id = fotoId });
        }

        public async Task<InspecaoFoto?> ObterFotoMaisRecenteAsync(int inspecaoId)
        {
            using var conn = _factory.CreateConnection();
            ...
            return await conn.QueryFirstOrDefaultAsync<InspecaoFoto>(sql, new { InspecaoId = inspecaoId });
        }
